...
export const PostingCardHorizontalMini: React.FC<PostingCardMiniProps> = ({ posting, onApply, hasApplied: hasAppliedProp }) => {
  ...
  return (
    <div className="bg-proph-grey rounded-2xl p-2 border border-proph-grey-text/20 w-full cursor-pointer" onClick={() => navigate(`/posting/${posting.id}`)}>
      <div className="flex gap-3 items-center">
        {/* Logo left */}
        {posting.school.logo && (
          <div className="flex-shrink-0 w-8 h-8 rounded-lg overflow-hidden bg-white/5 flex items-center justify-center">
            <img
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-0.5">
            <h3 className="text-sm font-bold text-proph-white truncate">{posting.school.name}</h3>
          ...
          {/* Position */}
          <h4 className="text-base font-extrabold text-proph-white">{posting.position}</h4>

          {/* Requirements line */}
          {reqString && (
            <p className="text-xs text-proph-grey-text mb-1">{reqString}</p>
          )}
          ...
          <div className="flex items-center justify-between">
            ...
            {hasApplied ? (
              <button disabled className="flex-shrink-0 bg-white/10 text-proph-yellow text-xs font-bold px-3 py-1.5 rounded-lg" onClick={(e) => e.stopPropagation()}>
                Applied
              </button>
            ) : (
              <button
                onClick={(e) => { e.stopPropagation(); onApply(posting.id); }}
                className="flex-shrink-0 bg-proph-yellow text-proph-black text-xs font-black px-3 py-1.5 rounded-lg hover:bg-[#E6D436] transition-colors"
              >
                APPLY
              </button>
            )}
